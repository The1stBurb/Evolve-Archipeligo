...
function payCosts(){return arguments}
...
//Build the requirment tree and num list
let techNums={}
let reqs={}
let deps={}
let specials={}
{
function rawName(type,num){return `${type}:${num}`}
let rawReqs={}
let rawToName={}
for(let i in techs){
    let tech=techs[i]
    let grant=rawName(tech.grant[0],tech.grant[1])
    rawToName[grant]=i
    rawReqs[grant]=[]
    for(let j in tech.reqs){
        let rnm=rawName(j,tech.reqs[j])
        // if(rawReqs.hasOwnProperty(rnm)){
        rawReqs[grant].push(rnm)
        // }
        // else{
            // rawReqs[rnm]=[grant]
        // }
...
function getRawName(nm){return rawToName.hasOwnProperty(nm)?rawToName[nm]:nm}
for(let i in rawReqs){
    let nm=rawToName[i]
    reqs[nm]=[]
    // console.log(i)
    if(techs[nm].hasOwnProperty("reqs")){
        techs[nm].reqs=rawReqs[i].map(reqnm=>getRawName(reqnm))
        rawReqs[i].forEach(reqnm=>{
            let realNm=getRawName(reqnm)
            realNm==reqnm?(specials.hasOwnProperty(nm)?specials[nm].push(realNm):specials[nm]=[realNm]):reqs[nm].push(realNm)
            // reqs[nm].push(realNm)
        })
    }
}
...
for(let i in reqs){
    reqs[i].forEach(req=>{
        deps.hasOwnProperty(req)?deps[req].push(i):deps[req]=[i]
    })
}

// console.log(techs["mad"])
// console.log(reqs)
...
for(let i in techs){
    ...
    if(tech.hasOwnProperty("trait")){
        traits[i]=[]
        tech.trait.forEach(trait=>{
            if(!excludeTraits.includes(trait))traits[i].push(trait)
        })
        if(traits[i].length==0)delete traits[i]
    }
    if(tech.hasOwnProperty("not_trait")){
        ntraits[i]=[]
        tech["not_trait"].forEach(trait=>{
            if(!excludeTraits.includes(trait))ntraits[i].push(trait)
        })
        if(ntraits[i].length==0)delete ntraits[i]
    }
}
}


//i think this is more reqs but for the races?
let race2={}
let genus={}
let genusFail={}
let cond_itms={}
function getAllReqs(name){
    let checked=[],marked=[],nmarked=[name],attempts=0;
    while(attempts<50&&nmarked.length!=0){
        attempts++
        ...
        marked.forEach(cm=>{
            // console.log(cm)
            if(deps.hasOwnProperty(cm))deps[cm].forEach(ct=>{
                if(!checked.includes(ct)&&!marked.includes(ct))nmarked.push(ct)
            })
        })
    }
...
for(var i in Races){
    ...
    for(var j in techs){
        let ct=techs[j]
        let skip=false
        if(ct.hasOwnProperty("trait")){
            for(var k in ct["trait"]){
                if(!Races[i].hasOwnProperty(ct["trait"][k])){
                    skip=true
                    break
                }
            }
        }
        if(ct.hasOwnProperty("not_trait")){
            for(var k in ct["not_trait"]){
                if(Races[i].hasOwnProperty(ct["not_trait"][k])){
                    skip=true
                    break
                }
            }
        }
        // if(ct.hasOwnProperty("condition")&&conds.hasOwnProperty(j)){
        //     if(!conds[j].evaluate({species:i,traits:Races[i]}))skip=true
        // }
        if(!skip){
            race[i].has.push(j)
        }
...
for(var i in race){
    let hasm=[]//race[i].has
    let nhasm=[]//race[i].nhas
    if(!(i!=="protoplasm"&&equalLists(race[i].has,race["protoplasm"].has))){
        for(var j in race[i].has){
            if(!race["protoplasm"].has.includes(race[i].has[j])){
                hasm.push(race[i].has[j])
            }
        }
        for(var j in race[i].nhas){
            if(!race["protoplasm"].nhas.includes(race[i].nhas[j])){
                nhasm.push(race[i].nhas[j])
            }
        }
    }
    race2[i]=new racer(i,races[i].type,hasm,nhasm)
}
for(let i in race2){
    let cr=race2[i]
    if(genus.hasOwnProperty(cr.genus)){
        let orace=race2[genus[cr.genus][0]]
        equalLists(orace.has,cr.has)&&equalLists(orace.nhas,cr.nhas)?genus[cr.genus].push(i):(genusFail.hasOwnProperty(cr.genus)?genusFail[cr.genus].push(i):genusFail[cr.genus]=[i])
    }
    else{
        genus[cr.genus]=[i]
    }
...
//make the failing race(s) be the lesser amount and construct the if things
let cond_itms_bad={}
for(let i in genus){
    if(genusFail.hasOwnProperty(i)&&genusFail[i].length>genus[i].length){
        let tmp=genusFail[i];
        genusFail[i]=genus[i]
        genus[i]=tmp
    }
    ...
    let grace=genus[i][0]
    if(i!=="avian"){
        race2[grace].nhas.forEach(req=>{
            let newreqs=getAllReqs(req)
            // console.log(newreqs,grace,req)
            newreqs.forEach(nreq=>{
                if(!race2[grace].nhas.includes(nreq)){
                    // console.log(nreq,grace)
                    race2[grace].nhas.push(nreq)
                }
            })
        })
    }
    race2[grace].has.forEach(req=>{
        let newreqs=getAllReqs(req)
        newreqs.forEach(nreq=>{
            if(!race2[grace].has.includes(nreq)){
                // console.log(nreq,grace)
                race2[grace].has.push(nreq)
            }
        })
    })
    if(race2[grace].nhas.length!=0||race2[grace].has.length!=0)cond_itms_bad[i]={"exclude":race2[grace].nhas,"include":race2[grace].has}
}
//and instead of redoing my code, fix it now!
for(let i in cond_itms_bad){
    cond_itms_bad[i].include.forEach(tech=>{
        cond_itms.hasOwnProperty(tech)?cond_itms[tech].include.push(i):cond_itms[tech]={"include":[i],"exclude":[]}
    })
    cond_itms_bad[i].exclude.forEach(tech=>{
        cond_itms.hasOwnProperty(tech)?cond_itms[tech].exclude.push(i):cond_itms[tech]={"include":[],"exclude":[i]}
    })
}
}

let nconds={}
//lets SOMEHOW do the conditions!
{
class OR{
    constructor(...args){
        this.lgcor=args.map(arg=>typerChecker(arg))
    }
    toString(){
        return "( "+this.lgcor.join(" || ")+" )"
    }
    evaluate(inp){
        // console.log(this.lgc)
        for(var i=0; i<this.lgcor.length; i++){
            var cl=this.lgcor[i];
            var out=null
            if(cl instanceof AND || cl instanceof OR){
                out=cl.evaluate(inp)
            }
            else{
                out=evaluater(cl,inp)
            }
            if(out)return true
        }
        return false
...
class AND{
    constructor(...args){
        this.lgcand=args.map(arg=>typerChecker(arg))
    }
    toString(){
        return "[ "+this.lgcand.join("&&")+" ]"
    }
    evaluate(inp){
        // console.log(this.lgc)
        for(var i=0; i<this.lgcand.length; i++){
            var cl=this.lgcand[i];
            var out=null
            if(cl instanceof AND || cl instanceof OR){
                out=cl.evaluate(inp)
            }
...
class Condition{
    constructor(cond){
        ...
        try{
            ...
            this.cond={build(){return ""}}
            console.error(`This condition failed to ifi ${cond}`)
        }
    }
    toString(){
        return cond
...
function evaluater(arg,inp){
    arg=bonk(arg)
    if(
        (arg[0]=="is_race"&&inp.traits.hasOwnProperty(arg[1][1]))||
        (arg[0]=="race_is"&&inp.species.hasOwnProperty(arg[1][2])&&(arg[1][1]=="=="||arg[1][1]=="==="))
    ){return true}
    return false
}
...
function conditionifi(cond){
    ...
    let attempts=0
    while(attempts<10){
        let closest=cond.indexOf("?")
        let closest2=cond.indexOf(":")
        if(closest==-1||closest2<closest){
            if(closest2==-1){
                sections[sections.length-1].onfalse=cond
                break
            }
            sections[sections.length-1].ontrue=cond.slice(0,closest2)
            cond=cond.slice(closest2+1)
            continue
        }
        sections.push(new Conditional(parse(cond.slice(0,closest))))
        cond=cond.slice(closest+1)
    }
    for(let i=sections.length-1; i>0; i--){
        sections[i-1].onfalse=sections[i]
    }
    return sections[0]
}
function bonk(arg){
    ...
    return [arg.slice(0,start),arg.slice(start+1,end).split(",")]
}
...
function parse(expr) {
    ...
    let orParts = splitTopLevel(expr, "||");
    if (orParts.length > 1) {
        return new OR(...orParts.map(parse));
    }

    // AND level
    let andParts = splitTopLevel(expr, "&&");
...
// fs.writeFileSync("archipelago/RacesHas.json", JSON.stringify(specials,"",2));
}